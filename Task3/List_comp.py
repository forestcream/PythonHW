...
result = [num ** 2 for num in range(2, 28, 2)]
...
result = [(digit_first + digit_second, digit_first - digit_second) for digit_first, digit_second in
          zip(nums_first, nums_second)]
...
in_file = """name,Petya,Vasya,Masha,Vova
grade,5,5,8,3
subject,math,language,physics,math
year,1999,2000,1995,1998"""
result = [{row.split(',')[0]: row.split(',')[i] for row in in_file.split('\n')} for i in range(1, len(in_file.split('\n')[0].split(',')))]
